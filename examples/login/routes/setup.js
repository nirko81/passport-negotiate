...
const authenticateLocation = "/authenticate-negotiate";
...
function hasAuthenticatedPrincipal(req, res, next) {
  console.log(
    "check for authenticated principal: " + req.session.authenticatedPrincipal
  );
...
module.exports = function (app, passport) {
  ...
  app.post("/manageprofile", hasAuthenticatedPrincipal, function (req, res) {
    let user = req.user;
    const principal = req.session.authenticatedPrincipal;
    ...
    user.save(function (err) {
      ...
      console.log(
        "User registration/update successful for principal: " + principal
      );
  ...
  app.post("/deleteprofile", isAuthenticated, function (req, res) {
    User.findOneAndRemove(
      { principal: req.session.authenticatedPrincipal },
      function (err) {
        if (err) {
  ...
  app.get(homeLocation, isAuthenticated, function (req, res) {
    console.log(
      "available credential cache: ",
      req.session.delegatedCredentialsCache
    );
